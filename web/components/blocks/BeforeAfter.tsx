...
const tag = 'absolute top-2 rounded bg-navy-deep/85 px-2 py-0.5 text-xs font-semibold text-white';
...
export function BeforeAfter({ before, after, caption }: { before: Img | null; after: Img | null; caption: string }) {
  ...
  return (
    <div className="relative h-full w-full select-none">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-y-0 w-0.5 -translate-x-1/2 bg-white shadow peer-focus-visible:w-1.5 peer-focus-visible:bg-orange"
        style={{ left: `${pos}%` }}
      >
        <span className="absolute top-1/2 left-1/2 h-9 w-9 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-navy-deep/80" />
      </div>
    </div>
  );
}

...
// S1 — Concept A hero structure (serif headline left, parquet panel right) in the docx palette; both CTAs above the fold.
  ...
  return (
    <section id="top" aria-label="Introduction" className="border-b border-line">
      <Container className="grid items-center gap-8 py-10 md:grid-cols-[1.05fr_0.95fr] md:gap-[clamp(32px,5vw,72px)] md:py-[clamp(56px,8vw,104px)]">
        <div>
          <p className="mb-6 text-[11.5px] font-semibold uppercase tracking-[0.24em] text-orange-text">{hero.eyebrow(site.startYear)}</p>
          <h1 className="font-display text-[clamp(38px,5.6vw,70px)] font-semibold leading-[1.03] tracking-[-0.028em] text-balance text-navy">
            {h.before}
            {/* Highlighted headline phrase in Syracuse Orange — large text, so the bright value is allowed (§5.4). */}
            <em className="italic text-orange">{h.highlight}</em>
            {h.after}
          </h1>
        ...
        <div className="relative aspect-[16/10] overflow-hidden rounded-[4px] bg-surface md:aspect-[1/1.02]">
          <Parquet />
          <div aria-hidden className="absolute inset-0 bg-gradient-to-br from-white/10 to-white/70" />
          <div className="absolute bottom-0 left-0 z-10 rounded-tr-[4px] bg-white px-5 pt-3.5 pb-3 text-[11.5px] font-medium uppercase tracking-[0.16em] text-stone">
            {hero.panelTag}
          </div>
        </div>
      </Container>
